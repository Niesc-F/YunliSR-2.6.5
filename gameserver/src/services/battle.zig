...
pub fn onStartCocoonStage(session: *Session, packet: *const Packet, allocator: Allocator) !void {
    ...
    battle.IPHBFHCEDGJ = config.battle_config.cycle_count; // cycle
    battle.KALINKEDDBE = @intCast(config.battle_config.monster_wave.items.len); // monster_wave_length
    ...
    try pfTargetHead.battle_target_list.append(.{ .Id = 10002, .Progress = 0, .TotalProgress = 0 });
    var pfTargetTail = protocol.BattleTargetList{ .battle_target_list = ArrayList(protocol.BattleTarget).init(allocator) };
    try pfTargetTail.battle_target_list.append(.{ .Id = 2001, .Progress = 0, .TotalProgress = 0 });
    try pfTargetTail.battle_target_list.append(.{ .Id = 2002, .Progress = 0, .TotalProgress = 0 });
    var asTargetHead = protocol.BattleTargetList{ .battle_target_list = ArrayList(protocol.BattleTarget).init(allocator) };
    try asTargetHead.battle_target_list.append(.{ .Id = 90005, .Progress = 0, .TotalProgress = 0 });
